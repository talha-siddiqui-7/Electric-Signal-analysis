...
full_file_path <- "G:/.shortcut-targets-by-id/1Hqecl4V9odrCM-fZun8l-1cDSCip00KD/ATELIER_Actividad_Tecnica/WP9/Info_IBERDROLA/FTP/mondragon/combined_data_mondragon.csv"
...
# Convert Date and Hour columns to a POSIXct DateTime using lubridate functions
avg_data <- avg_data %>%
  mutate(DateTime = dmy_hms(paste(Date, Hour)),
...
# Filter out rows where DateTime could not be parsed (i.e., resulting in NA)
avg_data <- avg_data %>%
...
# List of categories
categories <- c("GENERAL", "BATERAA AUTOMATICA", "CLIMA", "CUADRO TOMAS 1", "CUADRO TOMAS 2")
